...
def gen_scatterplot(_args):
    ...
    for index, (vessel_id, vessel_info) in enumerate(vessels_data.items(), start=1):
        if "navigation" in vessel_info and "position" in vessel_info["navigation"]:
            ...
            plt.annotate(f"Boat {index}", (longitude, latitude), textcoords="offset points", xytext=(0, 5), ha='center')
